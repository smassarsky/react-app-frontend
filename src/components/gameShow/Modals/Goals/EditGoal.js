...
import { periods, editGoalInitialState } from '../../../../config'

class EditGoal extends Component {

  state = {...editGoalInitialState}

  componentDidUpdate(prevProps) {
    if (this.props.goal && prevProps.goal !== this.props.goal) {
      const { assistPlayers, onIcePlayers, period, time } = this.props.goal
      const player = this.props.goal.player || {}
      const team = this.props.goal.team || {}
      const goalId = this.props.goal.id
      const [minutes, seconds] = time.split(':')
      this.setState( this.props.goal ? { player, assistPlayers, onIcePlayers, team, period, goalId, minutes, seconds } : editGoalInitialState)
    }
  }
  ...
  handleSubmit = e => {
    ...
    this.setState(editGoalInitialState)
  }
      ...
      <Modal show={this.props.show} onHide={this.handleHide} centered="true">
        <Modal.Body>
          <Form onSubmit={this.handleSubmit} >
            ...
            <InputError message={this.state.errors.team} />
            ...
            <Form.Row>
              <Form.Group as={Col}>
                ...
                >
                  <option value="-99">Reset Player</option>
                  {this.renderOptions()}
                </Form.Control>
              </Form.Group>
            </Form.Row>
            <InputError message={this.state.errors.player} />
            <Form.Row>
              <Form.Group as={Col}>
                <Form.Label className="text-muted mb-0">
                  <small>Assists</small>
